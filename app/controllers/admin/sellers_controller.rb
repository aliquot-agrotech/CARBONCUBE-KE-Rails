class Admin::SellersController < ApplicationController
  ...
  def index
    if params[:search_query].present?
      @vendors = Seller.where("phone_number = :query OR id = :query", query: params[:search_query])
    else
      @vendors = Seller.all
    end
  ...
  def reviews
    reviews = @vendor.reviews.joins(:ad, :buyer)
  ...
  def orders
    ...
    orders = @vendor.orders.includes(order_items: [:ad, :order], buyer: :orders)
  ...
  def fetch_analytics(vendor)
    ...
    reveal_vendor_details_clicks = click_event_counts["Reveal-Seller-Details"] || 0
